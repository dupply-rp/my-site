...
// apps/api/src/lib/consoleSession.ts
function getSecretKey() {
  const secret = process.env.CONSOLE_SECRET;
  if (!secret) return null;
  return new TextEncoder().encode(secret);
}
async function verifyConsoleToken(token) {
...
// apps/api/src/lib/consoleAuth.ts
function getBearerToken(header) {
  if (!header?.startsWith("Bearer ")) return null;
  return header.slice("Bearer ".length).trim() || null;
}
async function verifyConsoleAuthFromToken(authHeader) {
  const secret = process.env.CONSOLE_SECRET;
  if (!secret) return null;
  const token = getBearerToken(authHeader ?? void 0);
...
var diagnosticos = pgTable("diagnosticos", {
  id: uuid("id").primaryKey().defaultRandom(),
...
// apps/api/src/lib/consoleQueries.ts
function requireDb() {
  const db = createDb();
...
async function listDiagnosticos(tenantSlug, limit = 100) {
  const db = requireDb();
  const tenantId = await getTenantIdBySlug(tenantSlug);
  const conditions = tenantId ? eq(diagnosticos.tenantId, tenantId) : void 0;
  ...
  }).from(diagnosticos).orderBy(desc(diagnosticos.createdAt)).limit(limit);
  if (conditions) {
    return query.where(conditions);
  }
  return query;
}
async function getDiagnosticoById(tenantSlug, id) {
  const db = requireDb();
  const tenantId = await getTenantIdBySlug(tenantSlug);
  const conditions = tenantId ? and(eq(diagnosticos.id, id), eq(diagnosticos.tenantId, tenantId)) : eq(diagnosticos.id, id);
  ...
  }).from(diagnosticoRespostas).where(eq(diagnosticoRespostas.diagnosticoId, id));
  return {
    id: row.id,
...
async function handler(req, res) {
  ...
  try {
    if (id) {
      ...
      if (!diagnostico) {
        return res.status(404).json({ error: "Diagn\xF3stico n\xE3o encontrado" });
      }
    ...
    const message2 = error instanceof Error ? error.message : "Erro ao consultar banco";
