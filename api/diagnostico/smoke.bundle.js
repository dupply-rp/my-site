...
async function checkEmailSetup() {
  ...
  let recipientsSource = "none";
  try {
    const recipients = await resolveNotifyEmailAddresses();
    recipientCount = recipients.length;
    recipientsSource = recipientCount > 0 ? "database" : "none";
    if (recipientCount === 0) {
      issues.push("Nenhum destinat\xE1rio de notifica\xE7\xE3o \u2014 cadastre em /console/emails");
    }
  } catch (error) {
    const message = error instanceof Error ? error.message : "Erro ao ler destinat\xE1rios";
    console.warn("[email-setup] Fallback env:", message);
    const raw = sanitizeEnvValue(process.env.DIAGNOSTICO_NOTIFY_EMAILS);
    const fallback = raw ? raw.split(",").map((email) => email.trim()).filter(Boolean) : ["ricardo.lima@dupply.com.br", "ricardosllacerda@gmail.com"];
    recipientCount = fallback.length;
    recipientsSource = recipientCount > 0 ? "env" : "none";
    if (recipientCount === 0) {
      issues.push("Nenhum destinat\xE1rio em DIAGNOSTICO_NOTIFY_EMAILS");
    }
  }
  return {
...
async function sendTestNotificationEmail() {
  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      ...
      subject: "TC_teste \u2014 verifica\xE7\xE3o de e-mail Dupply",
      html: "<p>Smoke test: envio de e-mail em produ\xE7\xE3o OK.</p>"
    })
  });
  if (!response.ok) {
...
async function handler(req, res) {
  ...
  if (mode === "email") {
    ...
    const ok2 = setup.resendConfigured && setup.recipientCount > 0 && sendOk;
    return res.status(ok2 ? 200 : 503).json({
      ok: ok2,
      checks: {
        ...checks,
        emailSetup: setup,
